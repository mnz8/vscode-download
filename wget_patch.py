    ...
    try:
        ...
        def not_a_console(handle):
            if handle == INVALID_HANDLE_VALUE or handle is None:
                return True
            return ((GetFileType(handle) & ~FILE_TYPE_REMOTE) != FILE_TYPE_CHAR or GetConsoleMode(handle, byref(DWORD())) == 0)
...
def get_console_width():
    ...
    if os.name == 'nt':
        ...
        class CONSOLE_SCREEN_BUFFER_INFO(Structure):
            _fields_ = [("dwSize", COORD), ("dwCursorPosition", COORD), ("wAttributes", WORD), ("srWindow", SMALL_RECT), ("dwMaximumWindowSize", DWORD)]
...
def bar_adaptive(current, total, width=80, key=None, key_max_len=None):
    ...
    key_max_len = key_max_len if key_max_len else 30
    ...
    for field in selected:
        ...
        if key and field == 'key':
            output += key.ljust(min_width['key'])
        if field == 'percent':
...
def callback_progress(blocks, block_size, total_size, bar_function, key, key_max_len):
...
def download(url, out=None, key=None, key_max_len=None, bar=bar_adaptive):
